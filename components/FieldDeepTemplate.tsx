...
// ONE shared deep template for every industry/field page. Inline-editable: each text node is
// an <Editable> whose committed default comes from props, and whose override is stored per page
// (the page wraps this in <EditablePage pageKey="industry-...">). Arc:
//   the field & the playbook in it -> where it leaks -> the AI fix -> the M&A roll-up -> proof/CTA
export type FieldDeepProps = {
  name: string;
  tid?: string; // text-id prefix (unique per page; the page store also isolates by pageKey)
  eyebrow?: string;
  live?: boolean;
  intro?: string;
  leaksLede?: string;
      ...
      <section style={SECTION_LIGHT} className="w-full">
        <div className="mx-auto max-w-4xl px-6 pt-6 pb-16 text-center md:pt-10 md:pb-20">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-[color:var(--color-sjc-blue)]">
            <Editable tid={`${tid}.eyebrow`} as="span">{eyebrow}</Editable>
            {live && (
          ...
          <Editable
            tid={`${tid}.intro`}
            as="p"
            className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-[color:var(--color-sjc-ink)] md:text-xl"
          >
            {intro}
          </Editable>
        </div>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-4xl px-6 py-20 md:py-24">
          ...
          <Editable
            tid={`${tid}.leaks.lede`}
            as="p"
            className="mt-4 text-lg leading-relaxed text-[color:var(--color-sjc-ink)]"
          >
            {leaksLede}
          </Editable>
          <ul className="mt-6 space-y-3">
            {leaks.map((l, idx) => (
              <li key={idx} className="flex gap-3 text-lg leading-relaxed text-[color:var(--color-sjc-ink)]">
                ...
                <Editable tid={`${tid}.leak.${idx}`} as="span">{l}</Editable>
              </li>
            ))}
          </ul>
        </div>
      ...
      <section style={SECTION_LIGHT} className="w-full">
        <div className="mx-auto max-w-4xl px-6 py-20 md:py-24">
          ...
          <Editable
            tid={`${tid}.fix.p`}
            as="p"
            className="mt-6 text-lg leading-relaxed text-[color:var(--color-sjc-ink)]"
          >
            {fix}
          </Editable>
        </div>
