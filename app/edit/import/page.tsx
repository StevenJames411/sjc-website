...
export default function ImportPage() {
  ...
  const box: React.CSSProperties = {
    width: "100%", border: "1px solid #d1d5db", borderRadius: 8, padding: "10px 12px", fontSize: 14,
  };
  const btn: React.CSSProperties = {
    padding: "10px 18px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff",
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px 80px", fontFamily: "system-ui, sans-serif" }}>
      ...
      <label style={{ display: "block", fontWeight: 600, marginTop: 24, marginBottom: 6 }}>
        Business name <span style={{ fontWeight: 400, color: "#6b7280" }}>— becomes the web address</span>
      </label>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 16, alignItems: "center" }}>
        ...
        <button
          type="button"
          style={{ ...btn, background: "#111827", color: "#fff", border: "1px solid #111827" }}
          disabled={busy}
          onClick={() => run(false)}
        >
          Import as a new page
        </button>
        {busy ? <span style={{ color: "#6b7280", fontSize: 14 }}>working…</span> : null}
      </div>

      {res && !res.ok ? (
        <p style={{ marginTop: 20, color: "#b91c1c", fontWeight: 600 }}>{res.error}</p>
      ) : null}
      ...
      {res?.ok ? (
        <div style={{ marginTop: 28 }}>
              ...
              {ROLE_ROWS.map((r) => {
                ...
                return (
                  <tr key={r.key}>
                    ...
                    <td style={{ padding: "4px 10px" }}>
                      {hex ? (
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                          <span style={{ width: 18, height: 18, borderRadius: 4, background: hex, border: "1px solid #d1d5db" }} />
                          <code>{hex}</code>
                        </span>
                      ) : (
                    ...
                    <td style={{ padding: "4px 0", color: "#6b7280" }}>{r.why}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          <ul style={{ marginTop: 8, color: "#374151", fontSize: 13, lineHeight: 1.7 }}>
            {(res.report || []).map((line, i) => (
              <li key={i} style={{ color: line.includes("SKIPPED") ? "#b45309" : undefined }}>{line}</li>
            ))}
          </ul>
