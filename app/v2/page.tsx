...
import { Section, Rule, Eyebrow, H2, Lede } from "./Section";
    ...
    <main style={{ background: "var(--ink)", color: "#fff" }}>
        ...
        <div style={{ maxWidth: 900, margin: "clamp(38px,5vw,64px) auto 0" }}>
          {DIAGNOSIS.chain.map((c, i) => (
            <div key={c.k} style={{
              ...
              borderBottom: i < DIAGNOSIS.chain.length - 1 ? "1px solid rgba(var(--line),.08)" : "none",
            }}>
              <div style={{ fontFamily: "Georgia, serif", fontSize: "clamp(19px,1.7vw,25px)", whiteSpace: "nowrap", color: i === DIAGNOSIS.chain.length - 1 ? "var(--accent)" : "#fff" }}>{c.k}</div>
              <div style={{ color: "rgba(255,255,255,.62)", fontSize: "clamp(15px,1.15vw,17.5px)", lineHeight: 1.8, fontWeight: 300 }}>{c.d}</div>
            </div>
          ))}
        </div>

        <p style={{ maxWidth: "52ch", margin: "clamp(40px,5vw,64px) auto 0", textAlign: "center", fontFamily: "Georgia, serif", fontSize: "clamp(21px,2.2vw,32px)", lineHeight: 1.5, color: "#fff" }}>
          {DIAGNOSIS.closer}
        </p>
      </Section>
        ...
        <div style={{ display: "grid", gap: 20, maxWidth: 1180, margin: "clamp(40px,5vw,64px) auto 0", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))" }}>
          {SOLUTION.cards.map((s) => (
            <a key={s.n} href={s.href} className="sol-card" style={{
              ...
              border: "1px solid rgba(var(--line),.09)", padding: "32px 30px 34px",
            }}>
      ...
      <Section id="work">
        ...
        <div style={{ display: "grid", gap: 18, maxWidth: 1180, margin: "clamp(38px,5vw,60px) auto 0", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))" }}>
          {PROOF.items.map((w) => (
            <figure key={w.t} style={{ margin: 0, border: "1px solid rgba(var(--line),.09)", background: "var(--surface)" }}>
        ...
        <div style={{ maxWidth: 900, margin: "clamp(48px,6vw,80px) auto 0", border: "1px solid rgba(var(--accent-rgb),.3)", background: "rgba(var(--accent-rgb),.05)", padding: "clamp(28px,3.4vw,44px)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "clamp(24px,4vw,56px)", marginTop: 26 }}>
            {PROOF.caseStudy.stats.map((s) => (
              <div key={s.l}>
                <div style={{ fontFamily: "Georgia, serif", fontSize: "clamp(30px,3.2vw,44px)", color: "#fff" }}>{s.n}</div>
                <div style={{ color: "rgba(255,255,255,.55)", fontSize: 13.5, marginTop: 4, maxWidth: "22ch" }}>{s.l}</div>
              </div>
            ))}
          </div>
