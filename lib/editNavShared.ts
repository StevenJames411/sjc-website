// THE BACK OFFICE'S OWN MENU, AS DATA — the PURE half.
//
// ⚠️ NO STORAGE IN THIS FILE, and that is load-bearing. components/edit/navContext.tsx needs
// ENTRIES in the BROWSER, and ./editNav imports lib/store, which imports ioredis, which imports
// node:dns. One client import of the wrong file and the whole build dies on `Can't resolve 'dns'`.
// Same split, same reason, as checksShared / hostShared / sitesShared.
//
// ── WHY ───────────────────────────────────────────────────────────────────────────────────────
...
export const DEFAULT_DOC: NavDoc = {
  brand: DEFAULT_BRAND,
  entries: ENTRIES,
...
/**
 * WHAT ONE SCREEN IS CALLED — for its own heading and its browser tab.
 *
 * ⛔ ONE SCREEN, ONE NAME. Clicking "Global-Brand-Settings" and landing on a page headed "Brand"
 * costs half a second of "did I click the right thing" every single time. The heading reads from
 * the same document as the menu, keyed, so a rename in the rail moves both at once and there is
 * never a second place to edit.
 *
 * ⚠️ THIS IS NOT WHAT MAKES DEBUGGING WORK, and it was never meant to be. The anchor a bug report
 * hangs on is the URL — `/edit/brand` → `app/edit/brand/page.tsx` — and that is stable no matter
 * what either the rail or the heading says. The label is decoration; the key is identity. Renaming
 * a heading to match the menu makes the SCREEN coherent, not the codebase.
 *
 * ⚠️ Headings only. The sentence UNDER a heading stays in code: it explains what the page does,
 * and it has to keep doing that after Steven has renamed the page something personal.
 */
